// Hér koma reglur sem ég vill ekki hafa frá eslint

/* eslint-disable no-use-before-define */ // Fann ekki leið til þess að gera þetta án þess að brjóta þessa reglu
/* eslint-disable no-plusplus */ // Finnst þetta léleg regla
/* eslint-disable no-restricted-globals */ // Fann ekki aðra leið en að nota history
...
export function handleCategoryClick(id, container, newsItemLimit) {
  ...
  history.pushState(1, null, `/?category=${id}`);
  return (e) => {
    e.preventDefault();
 ...
 * @returns {function} Fall sem bundið er við click event á link/takka
 */
function handleBackClick(newsItemLimit) {
  const main = document.querySelector('.newsList__list');
  empty(main);
  fetchAndRenderLists(main, newsItemLimit);
...
export function createCategoryBackLink(container) {
  ...
  back.addEventListener('click', () => handleBackClick(5));
}
...
export async function fetchAndRenderLists(container, newsItemLimit) {
  ...
  for (let i = 0; i < newsItemLimit; i++) {
    const {id} = allt[i];
    // Gat ekki leyst þetta án þess að vera með await í for loop
    // eslint-disable-next-line no-await-in-loop
    await fetchAndRenderCategory(id, container, newsItemLimit);
    ...
    allNews.addEventListener('click', () =>
      handleCategoryClick(id, container, 20)
    );
  }
